...
const eventIdentifierList1 = fullTodoList1[fullTodoList1.length - 1].id;
  ...
  .add(
    ...
    async () => {
      await sorci.appendEvent({
        sourcingEvent: eventToPersist,
        query: {
          types: ["todo-list-created", "todo-list-item-created"],
          identifiers: [{ todoListId: todoList1Id }]
        },
        eventIdentifier: eventIdentifierList1
      });
      ...
      await Promise.all(
        renameEvents.map((item) =>
          sorci.appendEvent({
            ...
            query: {
              types: ["todo-list-created", "todo-list-deleted"],
              identifiers: [{ todoListId: item.todoListId }]
            },
            eventIdentifier: item.lastEventId
          })
        )
      );
    },
